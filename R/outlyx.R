...
### Computes outliers within methylomic datasets
 ...
 #        sample outlyingness.
 #
 # Returns:    A dataframe containing logical values where
...
###
  ...
  if(iqr){ # {{{
    ...
    df[["iqr"]] <- v1
    n <- n + 1
    low <- min(c(min(pccompbetx$rot[,pc]),out1[["low"]]))-out1[[2]]
    upp <- max(c(max(pccompbetx$rot[,pc]),out1[["hi"]]))+out1[[2]] 
  } # }}}
 ...
 # Output
  ...
  outy <- !apply(df,1, function(y)any(y==FALSE))
  # Constructing output file
  output <- list()
  output[['plotgubbins']] <- c(out1[["low"]], out1[["hi"]], low, upp, pc, n, iqrP, mvP)
  names(output[['plotgubbins']]) <- c('Lower Bound', 'Upper Bound',
                                     'Lowest Value', 'Highest Value',
                                     'Principal Component', 'Number of Tests',
                                     'IQRS', 'pcout Thresh')
  if(iqr) output[['pca']] <- pccompbetx$rot[,pc]
  if(mv)  output[['mv']] <- out2[[2]]
  output[['res']] <- df
  output[['outliers']] <- outy
  class(output) <- "outlyx.class"
  if(plot) plot(output) # Plot using plot method. 
  # FALSE = NOT OUTLYING
  return(output)
} # }}}

iqrFun <- function(x, pc, iqrP){ # {{{
  quantilesbx <- apply(x, 2, quantile)
...
setClass("outlyx.class")

print.outlyx.class <- function(x){ # {{{
  print(x[['outliers']])
  invisible(x)
} # }}}
...
setMethod("plot",
          "outlyx.class",
          function(x,...){
  if(!x[['plotgubbins']][6] == 2) stop("Not enough tests used!")
  b <- x[['plotgubbins']]
    plot(x[['pca']],
         x[['mv']],
         xlim= b[3:4],
